...
int pinNameToCode(String pinName) {
    pinName.toUpperCase();
    if (pinName.length() != 2 || pinName[0] != 'D') return -1;
    switch (pinName[1]) {
...
void updateSensorState(SensorID id, float value) {
    ...
    switch(id) {
        ...
        case SID_BATT_V: sensor.batteriespannung_v = value; break;
...
void checkHeapHealth() {
    ...
    if (freeHeap < MIN_HEAP_WARNING) { LOG(LOG_WARN, "⚠️ Heap niedrig: %d Bytes frei!", freeHeap); }
    if (freeHeap < 2000) { LOG(LOG_ERROR, "🚨 KRITISCH: Nur noch %d Bytes Heap! Neustart empfohlen.", freeHeap); }
}
...
bool loadConfig() { 
    ...
    if (configFile) {
        ...
        else { LOG(LOG_WARN, "Laden aus Datei fehlgeschlagen. Datei ist möglicherweise korrupt."); }
    }
