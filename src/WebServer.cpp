...
void setup_ota()
{
    ...
    {
        LOG(LOG_WARN, "OTA nicht gestartet: Kein WLAN.");
        return;
    }
...
void handleRoot()
{
    ...
    content += F("<p>Aktuelle RPM: ");
    content += String(rpm);
    content += F("</p>");
    content += F("<p>Max. Loop-Dauer (letzte Sekunde): ");
    content += String(max_loop_duration_us);
    content += F(" µs</p>");
    content += F("<p>Freier Heap: ");
    ...
    content += F("TPS (Raw): ");
    content += String(tps_raw);
    content += F(" | ");
    content += F("Geschw.: ");
    content += String(speed);
    content += F(" km/h | ");
    content += F("Batterie: ");
    content += String(batt_v, 1);
    content += F(" V | ");
    content += F("Temperatur: ");
    content += String(temp_kopf, 1);
    content += F(" °C</p>");
...
void handleViewLog()
{
    ...
    // Log-Level Selectbox
    content += F("<p>Aktueller Log-Level: ");
    content += String((int)activeLogLevel);
    content += F("</p>");
    content += F("Neuer Log-Level: <select name='level' id='logLevelSelect'>");
    content += F("<option value='0'");
    if (activeLogLevel == LOG_DEBUG)
        content += F(" selected");
    content += F(">0: DEBUG (Sehr ausführlich)</option>");
    content += F("<option value='1'");
    if (activeLogLevel == LOG_INFO)
        content += F(" selected");
    content += F(">1: INFO (Normaler Betrieb)</option>");
    content += F("<option value='2'");
    if (activeLogLevel == LOG_WARN)
        content += F(" selected");
    content += F(">2: WARNUNG (Auffälligkeiten)</option>");
    content += F("<option value='3'");
    if (activeLogLevel == LOG_ERROR)
        content += F(" selected");
    content += F(">3: FEHLER (Kritische Fehler)</option>");
    content += F("</select>");
    content += F("<br><br>");
...
void handleCurveConfig()
{
    ...
    html += F("<p>RPM Achse Punkte (X): ");
    html += String(rpm_size);
    html += F("</p>");
    html += F("<p>TPS Achse Punkte (Y): ");
    html += String(tps_size);
    html += F("</p>");
    html += F("<p>Daten Zeilen (Sollte TPS-Größe entsprechen): ");
    html += String(data_rows);
    html += F("</p>");
...
void handleRpmConfig()
{
    ...
    html += String(rpmConfig.pulses_per_revolution);
    html += F("' required><br>");
    ...
    html += String(timingConfig.trigger_offset_deg);
    html += F("' required><br>");
    ...
    html += String(timingConfig.tdc_offset_ms, 1);
    html += F("' required><br><br>");
